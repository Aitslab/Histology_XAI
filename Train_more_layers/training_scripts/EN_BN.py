...
def pretrained_model(img_shape, num_classes,layer_type):
    model_EN_conv = EfficientNetB4(weights='imagenet', include_top=False)

    #Input format
    keras_input = Input(shape=(224,224,3), name = 'image_input')
    
    #Use the generated model 
    output_EN_conv = model_EN_conv(keras_input)
    
    for layer in model_EN_conv.layers:
        layer.trainable = False
    model_EN_conv.layers[-2].trainable = True   ## This is the main logic for unfreezing deeper layer 
    model_EN_conv.layers[-3].trainable = True   ## Here, we unfreeze the Batch normalization layer + convolutional layer 

    print(model_EN_conv.layers[-1].trainable)

    #Add the fully-connected layers 
    x = Flatten(name='flatten')(output_EN_conv)
...
for val_s in validation_slide:
    ...
    train_DATA_PATH = '../analysed_data/All_samples_group_1_2_3_total_score_Median_aug/'+val_dir+'_val_total_score_cropped/train_aug/'  # This is the big augmented dataset
    ...
    #############################################################
	## Training the model
    model = pretrained_model(train_samples.shape[1:], 3,'relu') ## REGRESSION: number of classes 1 instead of 3
    model.summary()
